...
Note: Again, it is not recommended to install packages inside an R Markdown. We can do this in the console or in a separate .R script inside this R Project.
```{r, eval = FALSE}
...
complain more (e.g. when a variable does not exist).

This forces you to confront problems earlier, typically leading to
cleaner, more expressive code. T

ibbles also have an enhanced print() method which makes them easier to
...
Note that using \$ returns a vector while subsetting with [ keeps tibble
structure.
...
To explore basic dplyr vervs, we will use the `nycflights13::flights`
dataset. This dataset contains all 336,776 flights that departed from
New York City in 2013. The data comes from the US Bureau of
...
When we run `filter()` dplyr executes the filtering operation, creating
a new data frame, and then prints it.
...
appears on the left-hand side of the =, and the old variable appears on
the right-hand side
...
Results are NAs, R's symbol for missing value. This happened because
...
describing and building graphs. With ggplot2, you can do more and faster
...
The relationship appears to be positive: as flipper length increases, so
does body mass.

It also appears to be fairly linear (points are clustered around a line
instead of a curve), which looks moderately strong since there is not
too much scatter around the line.

It seems then that penguins with longer flippers are generally larger in
terms of their body mass.
...
1.  Loading all packages at the start of the script/R Markdown so that
    other users of our code can easily see what packages need to be
    installed.

2.  Installing packages through the console or a separate R script.

We do not really need to worry about installing packages if we enabled
renv when we initialized the R Project.
