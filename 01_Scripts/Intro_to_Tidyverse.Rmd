...
weather_2 <- read_csv(here("00_Data", "Syr_MetData.csv"))
...
inventory_tbl <- as_tibble(inventory)
...
weather %>% 
  filter(YEAR >= 1940 & YEAR <= 2000)

weather %>% 
  filter(YEAR %in% c(1940:2000))
...
### EXERCISE: Find how many distinct years there are in `weather` and how many occurrences there are of each year.
...
weather %>% 
  select(STATION, YEAR, PRCP, TMAX) %>% 
  filter(YEAR >= 1990) %>% 
  mutate(PRCP_cm = PRCP * 2.54) %>% 
  arrange(desc(PRCP))
...
### EXERCISE: Group the `weather` data per year and then get the total precipitation and mean maximum temperature. Then, create a column that converts the total precipitation in inches to millimeters.
...
weather |>
  ...
  mutate(total_PRCP_mm = total_PRCP * 25.4)
...
weather |>
  mutate(NAME_underscore = str_replace_all(NAME, "SYRACUSE", "SYR"))
...
weather |>
  mutate(STATION_SYR = str_c(STATION, "-SYR"))
