...
This forces you to confront problems earlier, typically leading to cleaner, more expressive code. T

ibbles also have an enhanced print() method which makes them easier to use with large datasets containing complex objects.
...
`tibble()` creates a tidy table, which automatically handles column types and prints nicely
...
as_tibble() converts data.frame to tibble for better readability and tidyverse compatibility.
...
```
Note that using $ returns a vector while subsetting with [ keeps tibble structure.
...
lubridate provides a set of functions for working with date-times, extending and improving on R's existing support for them.
...
dates1 <- ymd(c("2025-10-20", "2025-11-01", "2025-12-31"))  # ymd() parses strings in Year-Month-Day format
dates1
...
```
3. Creatomg a date column in tibbles/data frames
...
The following examples are lifted/modified from https://r4ds.hadley.nz/data-visualize
...
The penguins is a tibble tha contains 8 columns:
...
The ultimate goal in this chapter is to recreate the  visualization in https://r4ds.hadley.nz/data-visualize displaying the relationship between:

- flipper lengths and body masses + considering the species of the penguins
...
To tell ggplot() how the information from our data will be visually represented, we'll use the **mapping** argument to define the visual properties (**aesthetics**) of the plot.
...
```
The empty canvas now has more structure: flipper lengths will be displayed on the x-axis while body mass will be displayed on the y-axis.

But there's nothing else yet aside from those because we have not told ggplot how to represent the observations.

We can do that by defining the **geom**: the geometrical object that a plot uses to represent data. There are several types of functions that start with `geom_` in ggplot2:
...
The relationship appears to be positive: as flipper length increases, so does body mass.

It also appears to be fairly linear (points are clustered around a line instead of a curve), which looks moderately strong since there is not too much scatter around the line.

It seems then that penguins with longer flippers are generally larger in terms of their body mass.

What else can we do to this plot?

Can we display other relationships, as influenced by other variables?
...
When a categorical variable is mapped to an aesthetic, ggplot2 will automatically assign a unique value of aesthetic (ex. unique color) to each unique level of the variable -- a process also known as scaling.
...
```
This now almost looks like the target figure except that we have three separate lines for each penguin species instead of one.

Note that each geom function in ggplot2 can also take a mapping argument, which allows for aesthetic mappings at the local level, rather than inheriting from the global level (i.e., main mapping argument).
...
Almost there! Note that we can also use shape aesthetics to categorize data.
...
```
Lastly, we can export this plot for use in other purposes. To do this, first, the ggplot must be saved to an object:
...
-- This is a little difficult to understand and is used for more advanced applications involving combinations of various R programming concepts (e.g. lists that we did not really explore and master to a great extent).
...
1. Loading all packages at the start of the script/R Markdown so that other users of our code can easily see what packages need to be installed.

2. Installing packages through the console or a separate R script.

We do not really need to worry about installing packages if we enabled renv when we initialized the R Project. 

3. Using variable/data frame/tibble names that are easy to understand and can give an idea on what the data it contains is about.

4. Practice! There's a lot of tutorials available in the internet. Always go back to the basics to have a solid foundation of R syntax and programming concepts.
