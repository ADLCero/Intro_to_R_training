...
The tricky part in importing data sets to R is identifying the file path to the data.

To address this issue, there is a good package in R that we can use: `here`

## 5.1. `here` package

When we work on R projects, file paths often become messy especially when using `setwd()`
or absolute paths like "C:/Users/Amyel/Documents/project/data/data.csv".

This may break in another computer.

The `here` package solves this by automatically finding your project's root directory,
so you can refer to files relative to your project folder.
...
The resulting output shows you where you are in your R Projects directory.

For the next part, download the data set that was used in the previous training
(from the Google Drive or from here: 
...
```
Note: It's not really a good practice to install packages inside the R Markdown, we should install them instead in a separate .R file or in the console.

This is because every time we knot, R executes all chunks from top to bottom. It will re-install packages every time which can slow down the knitting, cause errors, and trigger CRAN mirror issues.
...
In case the code above does not work, you may need to re-authenticate your Google access:
1. remove any old tokens using:
gs4_deauth()

2. open browser and ask you to log in:
gs4_auth()      
